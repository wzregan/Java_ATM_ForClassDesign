...
public class OperationServerImpl {
	...
	public static void update(String username,Integer Operation,Double balance,String toname,String from)
	{
		...
		{
		case REGISTER_OPERATION:
			dao.InitOperation(username);
			mes=timeUtil.getTime()+" : "+"您注册了账号，欢迎您使用本银行系统";
			dao.OperationUpdate(username,mes);
			break;
		case DRAWMONEY_OPERATION:
			mes=timeUtil.getTime()+" : "+"您取出了"+balance+"元，余额还剩"+userdao.queryBalanceByname(username)+"元";
			dao.OperationUpdate(username,mes);
			break;
		case TRANSFER_OPERATION:
			mes=timeUtil.getTime()+" : "+"您向"+toname+"转账了"+balance+"元"+"，余额还剩"+userdao.queryBalanceByname(username)+"元";
			dao.OperationUpdate(username,mes);
			break;
		case SAVEMONEY_OPERATION:
			mes=timeUtil.getTime()+" : "+"您存入了"+balance+"元"+"，余额还剩"+userdao.queryBalanceByname(username)+"元";
			dao.OperationUpdate(username,mes);
			break;
		case GETMONEY_OPERATION:
			mes=timeUtil.getTime()+" : "+from+"向您转账了"+balance+"元"+"，余额还剩"+userdao.queryBalanceByname(username)+"元";
			dao.OperationUpdate(username,mes);
			break;
		}
